...
async function seedUpgrades() {
  ...
  const upgrades = upgradesData.upgrades.map((u: any) => ({
    id: u.id,
    name: u.name,
...
async function seedUpgradeRequirements() {
  ...
  await supabase.from('upgrade_requirements').delete().neq('id', 0);
...
async function seedAttachments() {
  ...
  const attachments = attachmentsData.attachments.map((a: any) => ({
    id: a.id,
    name: a.name,
    type: a.type,
    tier: a.tier,
